...
session_start();
if (isset($_POST['snake_ready'])) {
    $_SESSION['name'] = strtolower($_POST['snake_name']);
    include_once('check.php');
    $cell = mysqli_fetch_row(mysqli_query($conn, "SELECT `name` FROM `snake` WHERE `name`='" . $_SESSION['name'] . "'"))[0];
    if ($cell == null) {
        mysqli_query($conn, "INSERT INTO `snake`(`name`) VALUES ('" . $_SESSION['name'] . "')");
    }
...
<body>
    <div id='achievement' class='hidden'><img src="visual/endings/6.png" alt=""><at><atf><ah>Открыта новая концовка!<ab>???</ab></atf></at></div>
    <img src="visual/cave.png" alt="" id='cave' class='hidden'>
    <img src="" alt="" id='noise' class='hidden'>
    ...
    /*$conn= mysqli_connect('localhost', 'root', '', 'snaken');
    $name='hertles_graft';
    for ($i=1; $i <= 11; $i++) { 
        $ending[$i]=mysqli_fetch_row(mysqli_query($conn, "SELECT `ending".$i."` FROM `snake` WHERE `name`='".$name."'"))[0];
    }
    echo "<div class='Profile'><div class='nameOuter'>".$name."</div><div class='endingsOuter'>";
    for ($i=1; $i <= 11; $i++) { 
        if ($ending[$i]==1){
            echo "<img class='endingcard' src='visual/endings/".$i.".png'>";
        }
        if ($ending[$i]==0){
            echo "<img class='endingcard' src='visual/closed/".$i.".png'>";
        }
    }
    echo "</div></div>"*/
