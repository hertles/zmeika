...
function FriendInitialize() {
    ...
    friendquestion[1] = 'Что находится вниз по курсу?'
    friendquestionvar[1] = [
        'Там находится БЕЗДНА',
        'Одинокое, темное, и холодное место для самых отчаявшихся в своем бытии',
        'Ты сам можешь его посетить, хотя я не советую... Не факт, что ты вернешься',
        'Я бы дал тебе фонарик, но он разрядился',
        'Ты лучше не иди туда, останься здесь',
        'question'
    ]
    ...
    friendquestionvar[2] = [
        ...
        'Но ты мне сломал стену, а ее восстановление для меня будет дороговато...',
    ...
    if (op == 'friend') {
        ...
        if (friendquestion[variant]=='У меня есть 5  <goldenspan></goldenspan>'){
            ...
            if (extrafoodEaten>=10){
                ...
                document.querySelector('.x50.y281').classList.remove('friendfood')
                document.querySelector('.x50.y281').classList.add('not_a_drug')
                document.querySelector('.x50.y281').setAttribute('Состав','Для создания такого корма неизвестность замешивается с неизвестностью и добавляется неизвестность (конечно же, в виде порошка)')
            }
        }
...
window.onload = function SnakeGame() {
    ...
    var TalkWithDevil = (e) => {
        ...
        if (devilreplic[devilreplicnumber]=='kitchen'){
            ...
            setTimeout(() => {
                window.location.href = 'HellEnding.php';
            }, 12000);
  ...
  console.log(data.ip);
    ...
    function FriendFood(food) {
        ...
        if (document.querySelectorAll('.friendfood').length==0){
            while (spawned==0) {
                let xf = 1 + Math.floor((Math.random()) * 59);
                let yf = 269 + Math.floor((Math.random()) * 28);
                if (Math.random()<0.2){
                    document.querySelector('.x10.y277').classList.add('friendfood')
                    spawned=1;
                }
                else{
    ...
    function TripToBottom() {
        ...
        if (y > 59) {
            document.querySelector('#bluegrad')
            document.querySelector('.x1' + '.y58').scrollIntoView();
        }
        if (y > 89) {
            document.querySelector('.x1' + '.y88').scrollIntoView();
        }
        if (y > 119) {
            document.querySelector('.x1' + '.y118').scrollIntoView();
            document.querySelector('#bluegrad').style.opacity = 0.5
        }
        if (y == 119) {
        ...
        if (y > 149) {
            document.querySelector('.x1' + '.y148').scrollIntoView();
        }
        if (y > 179) {
            document.querySelector('.x1' + '.y178').scrollIntoView();
            ShowMessage(message[3])
            message[3] = 0
        }
        if (y > 209) {
            document.querySelector('.x1' + '.y208').scrollIntoView();
        }
        if (y > 239) {
            document.querySelector('.x1' + '.y238').scrollIntoView();
        ...
        if (y > 269) {
            document.querySelector('.x1' + '.y268').scrollIntoView();
        }
        if (y > 269 && y < 299) {

        }
        if (y == 269 && direction == 'down' || y == 299 && direction == 'up') {
            friend[0] = document.querySelector('.x10.y276')
            friend[1] = document.querySelector('.x9.y276')
            friend[2] = document.querySelector('.x9.y277')
            friend[3] = document.querySelector('.x9.y278')
            friend[4] = document.querySelector('.x10.y278')
            friend[5] = document.querySelector('.x11.y278')
            friend[6] = document.querySelector('.x12.y278')
            friend.forEach((el) => { el.classList.add('friend'); el.setAttribute('Состав','Привет! Меня зовут Змейка') })
            document.querySelector('.x50.y281').classList.add('friendfood')
            opponent='friend'
            document.addEventListener('keydown', TalkWithFriend)
            document.querySelector('#entermessage').classList.remove('hidden');
        }
        if (y == 269 && direction == 'up' || y == 299 && direction == 'down') {

            document.removeEventListener('keydown', TalkWithFriend)
            document.querySelector('#entermessage').classList.add('hidden');
        }
        if (y > 299) {
            document.querySelector('.x1' + '.y298').scrollIntoView();
        }
    ...
    function FriendNextBlock(dir) {
        ...
        if (document.querySelector('.x' + (xfr) + '.y' + yfr).classList.contains('block') || document.querySelector('.x' + (xfr) + '.y' + yfr).classList.contains('invblock') || document.querySelector('.x' + (xfr) + '.y' + yfr).classList.contains('friend')) {
            frienddead=1;
