...
friendquestionvar[0] = [
    ...
    'Может, есть способ прочитать его мысли?',
    'question'
]
...
friendquestionvar[2] = [
    ...
    'Может, можно подмешать что-нибудь туда?',
    'Но давай так: сделай это, если ты точно уверен в своих словах',
    'question'
]
friendquestion[3] = 'Прости, что так с тобой поступил'
friendquestionvar[3] = [
    'Слушай, не вини себя',
    'Смерть это не самое худшее, что может произойти',
    'Тем более я сам чуть тебя не обрек на нее',
    'Раньше я, как и ты, тоже боялся смерти',
    'Но после некоторых событий я изменил свое мнение обо многих вещах',
    'Так что не беспокойся, я на тебя не в обиде :)',
    'question'
]
...
window.onload = function SnakeGame() {
    ...
    function HellEnding() {
        ...
        document.querySelectorAll('td:not(.snake)').forEach((el) => { el.classList.remove('hiddencell'); el.classList.add('hiddenblock'); el.classList.remove('block') })
    ...
    function FriendsEndingFinal() {
        ...
        for (let yy = 1; yy <= 29; yy++) {
            for (let xx = 1; xx <= 60; xx++) {
                ...
                document.querySelector('.x' + xx + '.y' + yy).classList.add('hiddenblock')
            }
        }
        ...
        if (devilreplic[devilreplicnumber + 1] == 'continue') {
            ...
            switch (tryed) {
                ...
                case 6:
                    ...
                    devil[devilreplicnumber + 1] = 2
                    devilreplic.push('Потому что даже не думал вглядеться в корм')
                    devil.push(2)
                    devilreplic.push('Всего-то надо было внимательно его изучить(!)')
                    devil.push(2)
                    devilreplic.push('Изучить изнутри')
                    devil.push(2)
    ...
    function Thoughts() {
        ...
        ThoughtsInterval = setInterval(() => {
            ...
            if (random < 0.1) {
                ...
                thought = 'Мы все состоим из кода. Даже корм! Вот это да...'
            }
            else if (random < 0.3) {
                ...
                thought = 'Просмотр кода элемента... Просмотр кода элемента...'
            }
            else if (random < 1) {
    ...
    endingmessage[10] = 'И чтобы понять это... давайте же вспомним, с чего все начиналось'
    endingmessage[11] = 'end'
        ...
        try {
            if (document.querySelector('.x' + (x) + '.y' + y).classList.contains('block') || document.querySelector('.x' + (x) + '.y' + y).classList.contains('snake') || document.querySelector('.x' + (x) + '.y' + y).classList.contains('kitchenblock') || document.querySelector('.x' + (x) + '.y' + y).classList.contains('cat') || document.querySelector('.x' + (x) + '.y' + y).classList.contains('ice')) {
                ...
                if (document.querySelector('.x' + (x) + '.y' + y).classList.contains('yellow')) {
                    ...
                    yellow = true
                    document.removeEventListener('keydown', FreeMovekeypressed)
                    document.addEventListener('keydown', yellowMovekeypressed)
                    let audio = new Audio();
    ...
    function yellowMovekeypressed(e) {
        let key = `${e.code}`;
        switch (key) {
            case 'ArrowLeft':
                lastx = x
